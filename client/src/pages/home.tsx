...
import { Button, Skeleton, Carousel, CarouselContent, CarouselItem, CarouselPrevious, CarouselNext } from "@/components/ui";
              ...
              slides.map((slide) => {
                ...
                return (
                  <CarouselItem key={slide.id}>
                    <Link href={(slide.buttonUrl as string) || '/'} className="block">
                      <div 
                        className={`bg-gradient-to-l from-${(slide as any).gradientFrom || 'primary/20'} to-${(slide as any).gradientTo || 'secondary/20'} rounded-xl overflow-hidden cursor-pointer hover:shadow-lg transition-shadow`}
                      >
                        <div className="flex flex-col md:flex-row items-center justify-between p-6 md:p-10">
                          ...
                          <div className="flex items-center justify-center">
                            {slide.imageUrl ? (
                              ...
                              <div className="bg-white rounded-full h-48 w-48 p-3 shadow-lg">
                                <div className={`bg-gradient-to-br from-${((slide as any).gradientFrom as string | undefined)?.replace('/20', '/10') || 'primary/10'} to-${((slide as any).gradientTo as string | undefined)?.replace('/20', '/30') || 'primary/30'} h-full w-full rounded-full flex items-center justify-center`}>
