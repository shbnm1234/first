...
function WebinarsManagerTab() {
  ...
  // add optional fields used elsewhere
  // ensure createData has thumbnailUrl and tags to avoid index errors
  // (these are optional and can be empty strings)
  // Note: keep the minimal shape to avoid widening types
  // when setting state we spread createData so these keys will exist
  if ((createData as any).thumbnailUrl === undefined) {
    (createData as any).thumbnailUrl = '';
  }
  if ((createData as any).tags === undefined) {
    (createData as any).tags = '';
  }
...
function DocumentsTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
  <h2 className="text-2xl font-bold text-gray-900">آرشیو مقالات</h2>
        <button
          onClick={() => setShowCreateForm(true)}
...
function ArticlesTab() {
  ...
  // categories for articles/documents
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      setCreateData(prev => ({ ...prev, content: html }));
    }
  });
  ...
  const handleCreateSubmit = () => {
    ...
    if (editor) {
      createData.content = editor.getHTML();
    }
  ...
  const handleUpdateSubmit = () => {
    if (editor && editingArticle) {
      editingArticle.content = editor.getHTML();
    }
  ...
  const handleEdit = (article: any) => {
    ...
    // populate editor with article content
    setTimeout(() => {
      if (editor && article && article.content) {
        ...
        <div className="bg-white rounded-lg border p-6">
          ...
          <div className="space-y-4">
                ...
                <div className="border-b p-2 bg-gray-50 flex items-center gap-2">
                  <button
                    ...
                    onClick={() => {
                      ...
                      const el = document.getElementById('article-image-upload') as HTMLInputElement | null;
                      el?.click();
                    }}
                    className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center gap-1"
                  >
                  ...
                  <input id="article-image-upload" type="file" accept="image/*" className="hidden" onChange={async (e) => {
                    ...
                    try {
                      ...
                      const url = j.files && j.files[0] && j.files[0].url ? j.files[0].url : j.url || j.file?.url;
                      if (!url) { alert('آدرس فایل پیدا نشد'); return; }
                      // insert into editor
                  ...
                  <button
                    type="button"
                    onClick={() => alert('برای افزودن ویدیو یا دیگر مدیاها، ابتدا آن‌ها را در کتابخانه رسانه آپلود کنید و سپس آدرس را وارد کنید.')}
                    className="px-3 py-1 text-sm bg-purple-600 text-white rounded hover:bg-purple-700 flex items-center gap-1"
                  >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">دسته‌بندی</label>
              <select
                value={(createData as any).category || ''}
                onChange={(e) => setCreateData({...createData, category: e.target.value})}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <input
                type="date"
                value={(createData as any).publishDate ? new Date((createData as any).publishDate).toISOString().slice(0,10) : ''}
                onChange={(e) => setCreateData({...createData, publishDate: e.target.value})}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            <div className="mt-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">یا آپلود تصویر شاخص</label>
              <div className="flex items-center gap-2">
                <input id="featured-image-upload" type="file" accept="image/*" className="hidden" onChange={async (e) => {
                  const file = e.target.files?.[0];
                  if (!file) return;
                  try {
                    const form = new FormData();
                    ...
                    const url = j.files && j.files[0] && j.files[0].url ? j.files[0].url : j.url || j.file?.url;
                    if (!url) { alert('آدرس فایل پیدا نشد'); return; }
                    setCreateData(prev => ({ ...prev, featuredImageUrl: url }));
                  } catch (err) {
                    console.error(err);
                    alert('خطا در آپلود تصویر');
                  } finally { e.currentTarget.value = ''; }
                }} />
                <button type="button" onClick={() => { const el = document.getElementById('featured-image-upload') as HTMLInputElement | null; el?.click(); }} className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">آپلود</button>
                {(createData as any).featuredImageUrl && <img src={(createData as any).featuredImageUrl} alt="preview" className="h-12 rounded" />}
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
                ...
                <div className="border rounded-lg">
                  <div className="border-b p-2 bg-gray-50 flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        const el = document.getElementById('article-image-upload-edit') as HTMLInputElement | null;
                        el?.click();
                      }}
                      className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center gap-1"
                    >
                    ...
                    <input id="article-image-upload-edit" type="file" accept="image/*,video/*,application/pdf" className="hidden" onChange={async (e) => {
                      const file = e.target.files?.[0];
                      if (!file) return;
                      try {
                        const form = new FormData();
                        ...
                        const url = j.files && j.files[0] && j.files[0].url ? j.files[0].url : j.url || j.file?.url;
                        if (!url) { alert('آدرس فایل پیدا نشد'); return; }
                        const lc = (file.name || url).toLowerCase();
                        if (lc.endsWith('.mp4') || lc.endsWith('.webm') || lc.endsWith('.ogg')) {
                          editor?.chain().focus().insertContent(`<video controls src="${url}" style="max-width:100%"></video>`).run();
                        } else if (lc.endsWith('.pdf')) {
                          editor?.chain().focus().insertContent(`<a href="${url}" target="_blank" rel="noopener">${file.name || 'PDF'}</a>`).run();
                        } else if (j.files && j.files[0] && j.files[0].type && j.files[0].type.startsWith('image')) {
                          editor?.chain().focus().setImage({ src: url }).run();
                        } else {
                          // fallback to link
                          editor?.chain().focus().insertContent(`<a href="${url}" target="_blank" rel="noopener">${file.name || url}</a>`).run();
                        }
                      } catch (err) {
                        console.error(err);
                        alert('خطا در آپلود فایل');
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const el = document.getElementById('featured-image-upload-edit') as HTMLInputElement | null;
                      el?.click();
                    }}
                ...
                <input id="featured-image-upload-edit" type="file" accept="image/*" className="hidden" onChange={async (e) => {
                  const file = e.target.files?.[0];
                  if (!file) return;
                  try {
                    const form = new FormData();
                    ...
                    const url = j.files && j.files[0] && j.files[0].url ? j.files[0].url : j.url || j.file?.url;
                    if (!url) { alert('آدرس فایل پیدا نشد'); return; }
                    setEditingArticle((prev: any) => ({ ...prev, featuredImageUrl: url }));
                  } catch (err) {
                    console.error(err);
                    alert('خطا در آپلود تصویر');
                  } finally { e.currentTarget.value = ''; }
              ...
              <div>
                ...
                <input
                  type="date"
                  value={editingArticle.publishDate ? new Date(editingArticle.publishDate).toISOString().slice(0,10) : ''}
...
function EducationalVideosTab() {
  ...
  const [createData, setCreateData] = useState<EduVideoCreateData>({
    title: '',
    videoUrl: '',
    orderPosition: 0,
    thumbnailUrl: '',
  ...
  const handleCreateSubmit = () => {
    ...
    const tagsArray = createData.tags ? createData.tags.split(',').map((tag: string) => tag.trim()).filter((tag: string) => tag) : [];
