...
export default function HomePage() {
  ...
  const { data: workshops } = useQuery<any[]>({
    ...
    queryFn: () => fetch('/api/workshops').then(res => res.json()),
  });

  const { data: webinars } = useQuery<any[]>({
    queryKey: ['/api/webinars'],
    queryFn: () => fetch('/api/webinars').then(res => res.json()),
  });

  const { data: educationalVideos } = useQuery<any[]>({
    queryKey: ['/api/educational-videos/active'],
    queryFn: () => fetch('/api/educational-videos/active').then(res => res.json()),
  });
          ...
          <div key={item.id} className="text-center">
            <button 
              onClick={() => {
                ...
                } else if (item.title === 'آرشیو مقالات' || item.title === 'کتابخانه' || item.title.includes('آرشیف') || item.title.includes('کتابخانه')) {
