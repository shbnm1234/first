...
export default function DocumentViewerPage() {
  ...
  const docForViewer = {
    ...document,
    // PDFViewer expects a lastUpdated string — provide a safe fallback
    lastUpdated: (document as any)?.lastUpdated || new Date().toLocaleDateString('fa-IR'),
    fileName: (document as any)?.title || 'document.pdf',
    fileUrl: (document as any)?.linkUrl || ''
  };

  return (
    <PDFViewer document={docForViewer as any} />
  );
}
