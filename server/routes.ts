...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/register', async (req, res) => {
    try {
      ...
      // Require username and password; name/email are optional
      if (!username || !password) {
        return res.status(400).json({ error: 'نام کاربری و رمز عبور الزامی است' });
      }
      ...
      // Create user (fill missing optional fields with null or sensible defaults)
      ...
      // Store user in session (normalize optional fields to empty string for session)
      req.session.user = {
        id: user.id,
        username: user.username,
        role: user.role ?? 'user',
  ...
  // Featured Documents (summaries) API
  app.get('/api/documents/featured', async (req, res) => {
    try {
  ...
  // Partial update (PATCH) for documents (e.g., toggle isFeatured)
